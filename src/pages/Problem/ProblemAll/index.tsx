...
import {getProblemTags, listQuestionVoByPageUsingPost} from "@/services/problem/api";
...
const SafeProblemTable: React.FC = ()=>{
  ...
  const reloadData = ()=>{
    ...
    listQuestionVoByPageUsingPost({
      ...
      if(res.code === 0){
        ...
        setLoading(false); // 即使请求失败，也要设置 loading 为 false
      }
    })
  }
  ...
  const columns: ProColumns<Problem.SafeProblem>[] = [
        ...
        <>
          {
            ...
            problem.status === '未开始' && <CheckCircleOutlined style={{fontSize: 18, color: Color.HARD}}/>
          }
        </>
      ),
    },
    ...
    },
    {
      ...
      render: (dom, entity) => {
        return (<>
          {/*<Tooltip placement="top" title="跳转在线做题页面" color="#FA541C">*/}
          {/*  <Button  onClick={()=>clickInspect(entity.id)} type="primary" icon={<IconFont type='icon-chakan'/>} >挑战</Button>*/}
          {/*</Tooltip>*/}
          <Button  onClick={()=>clickInspect(entity.id)} type="default" icon={<IconFont type='icon-chakan'/>} style={{ display: 'flex', alignItems: 'center' }}>
            <Space>
